/*
	pour la creation d'un compte (EnregistrerClient) :
	
	PasswordAuthentication psw = new PasswordAuthentication(5);
	String token = psw.hash(super.getPassword().toCharArray());
	
	on enregistre alors le client avec le String token a la place du mot de passe en clair
	
	pour l'autentification (ConnexionClient) :
	
	
	PasswordAuthentication psw = new PasswordAuthentication(5);
	if(psw.authenticate(mdp.toCharArray(),resu.getString("password"))){
		System.out.println("meme mot de passe");
	}else{
